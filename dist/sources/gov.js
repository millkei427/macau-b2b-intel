...
const APM_INDEX_URL = "https://www.gov.mo/zh-hant/about-government/apm/apm-entity-index/";
const HEADERS = { "User-Agent": "Mozilla/5.0 MacauB2BIntel/0.1" };
async function fetchWithRetry(url, attempts = 3, timeoutMs = 45000) {
 ...
 */
export async function fetchAllMacauGovEntities() {
    const html = await fetchWithRetry(APM_INDEX_URL);
    if (!html)
        throw new Error("gov.mo APM fetch failed after retries");
    ...
    $('a[alt="網站"]').each((_, webA) => {
        ...
        if (!website || !/\.gov\.mo/i.test(website))
            return;
 ...
 */
export function filterTenderLikely(entities) {
